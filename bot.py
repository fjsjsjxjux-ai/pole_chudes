...
def kb_categories(prefix: str = "cat") -> InlineKeyboardMarkup:
    """Используем индексы вместо полных названий — callback_data не может превышать 64 байта."""
    buttons = []
    row = []
    for i, cat in enumerate(ALL_CATEGORIES):
        row.append(InlineKeyboardButton(text=cat, callback_data=f"{prefix}_{i}"))
        if len(row) == 2:
            buttons.append(row); row = []
    if row: buttons.append(row)
    buttons.append([InlineKeyboardButton(text="🎲 Случайная", callback_data=f"{prefix}_r")])
    buttons.append([InlineKeyboardButton(text="🏠 Меню", callback_data="main_menu")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

def _decode_cat(raw: str) -> str:
    """Декодирует индекс категории обратно в название. 'r' = случайная."""
    if raw == "r":
        return random.choice(ALL_CATEGORIES)
    try:
        return ALL_CATEGORIES[int(raw)]
